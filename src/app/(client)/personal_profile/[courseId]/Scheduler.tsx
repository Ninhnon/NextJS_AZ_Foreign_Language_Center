...
export default function Scheduler({ courseId }) {
  ...
  const detailOptions = [
    ...
    { id: 2, text: 'Danh sách bài tập' },
    { id: 3, text: 'Kết quả học tập' },
  ];
          ...
          <div className="w-fit h-fit pb-2 flex flex-row">
            {detailOptions.map((button) => (
              <Button
                ...
                onClick={() => {
                  if (button.id === 1) {
                    router.push(`/personal_profile/${data?.id}`);
                  } else if (button.id === 2) {
                    router.push(
                      `/personal_profile/${data?.id}/assignment?id=${data?.id}`
                    );
                  } else if (button.id === 3) {
                    router.push(`/personal_profile/${data?.id}/result`);
                  }
                }}
              >
                {button.text}
